...
mixin _$RoomListItem {
 ...
 String get id; String? get name; String? get subject; String? get avatarUrl;/// Text of the last message exactly as its sender wrote it, and nothing
...
/// `lastMessage*` fields below — see `buildLastMessagePreview`.
 String? get lastMessage; DateTime? get lastMessageTime; String? get lastMessageUserId;/// Display name of [lastMessageUserId], resolved by the adapter from
...
/// also what rows persisted before this field existed rehydrate to.
 bool get lastMessageIsSystem; String? get lastMessageReactionEmoji;/// Text of the message the last reaction was aimed at, as its own
...
/// was not in reach when the reaction landed.
 MessageType? get lastMessageReactionTargetType; int get unreadCount;/// Count of unread messages in this room that mention the current user.
...
/// preference). Drives the read-only composer via [isReadOnly].
 bool get selfMuted;/// Mirrors [RoomConfig.writePolicy]: who may post into this room. Read
...
/// also what an unrecognised or absent value resolves to.
 RoomWritePolicy get writePolicy; bool get isGroup; bool get isAnnouncement; bool? get isOnline; PresenceStatus? get presenceStatus;/// Wall-clock time the other 1:1 participant was last seen online,
...
/// subtitle in [ChatRoomAppBar] when [isOnline] is `false`.
 DateTime? get lastSeen; String? get otherUserId; RoomRole? get userRole; int? get memberCount; Map<String, dynamic>? get custom; Set<String> get typingUserIds;/// Title computed by the adapter via the configured `RoomTitleResolver` or
...
as MessageType?,unreadCount: null == unreadCount ? _self.unreadCount : unreadCount // ignore: cast_nullable_to_non_nullable
...
as RoomWritePolicy,isGroup: null == isGroup ? _self.isGroup : isGroup // ignore: cast_nullable_to_non_nullable
...
as PresenceStatus?,lastSeen: freezed == lastSeen ? _self.lastSeen : lastSeen // ignore: cast_nullable_to_non_nullable
...
switch (_that) {
case _RoomListItem() when $default != null:
return $default(_that.id,_that.name,_that.subject,_that.avatarUrl,_that.lastMessage,_that.lastMessageTime,_that.lastMessageUserId,_that.lastMessageSenderName,_that.lastMessageId,_that.lastMessageReceipt,_that.lastMessageType,_that.lastMessageMimeType,_that.lastMessageFileName,_that.lastMessageDurationMs,_that.lastMessageIsDeleted,_that.lastMessageIsSystem,_that.lastMessageReactionEmoji,_that.lastMessageReactionTargetText,_that.lastMessageReactionTargetType,_that.unreadCount,_that.unreadMentions,_that.muted,_that.muteUntil,_that.pinned,_that.hidden,_that.selfMuted,_that.writePolicy,_that.isGroup,_that.isAnnouncement,_that.isOnline,_that.presenceStatus,_that.lastSeen,_that.otherUserId,_that.userRole,_that.memberCount,_that.custom,_that.typingUserIds,_that.effectiveDisplayName,_that.isParticipating);case _:
  return orElse();
...
class _RoomListItem extends RoomListItem {
  const _RoomListItem({required this.id, this.name, this.subject, this.avatarUrl, this.lastMessage, this.lastMessageTime, this.lastMessageUserId, this.lastMessageSenderName, this.lastMessageId, this.lastMessageReceipt, this.lastMessageType, this.lastMessageMimeType, this.lastMessageFileName, this.lastMessageDurationMs, this.lastMessageIsDeleted = false, this.lastMessageIsSystem = false, this.lastMessageReactionEmoji, this.lastMessageReactionTargetText, this.lastMessageReactionTargetType, this.unreadCount = 0, this.unreadMentions = 0, this.muted = false, this.muteUntil, this.pinned = false, this.hidden = false, this.selfMuted = false, this.writePolicy = RoomWritePolicy.members, this.isGroup = false, this.isAnnouncement = false, this.isOnline, this.presenceStatus, this.lastSeen, this.otherUserId, this.userRole, this.memberCount, final  Map<String, dynamic>? custom, final  Set<String> typingUserIds = const <String>{}, this.effectiveDisplayName, this.isParticipating = true}): _custom = custom,_typingUserIds = typingUserIds,super._();
...
@override
int get hashCode => Object.hashAll([runtimeType,id,name,subject,avatarUrl,lastMessage,lastMessageTime,lastMessageUserId,lastMessageSenderName,lastMessageId,lastMessageReceipt,lastMessageType,lastMessageMimeType,lastMessageFileName,lastMessageDurationMs,lastMessageIsDeleted,lastMessageIsSystem,lastMessageReactionEmoji,lastMessageReactionTargetText,lastMessageReactionTargetType,unreadCount,unreadMentions,muted,muteUntil,pinned,hidden,selfMuted,writePolicy,isGroup,isAnnouncement,isOnline,presenceStatus,lastSeen,otherUserId,userRole,memberCount,const DeepCollectionEquality().hash(_custom),const DeepCollectionEquality().hash(_typingUserIds),effectiveDisplayName,isParticipating]);
