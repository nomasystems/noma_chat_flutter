...
void main() {
  ...
  group('MessageBubble', () {
    ...
    testWidgets('renders system message as centered text', (tester) async {
      ...
      await tester.pumpWidget(wrap(
        MessageBubble(message: msg, isOutgoing: false),
      ));
    ...
    testWidgets('reduced top padding when isFirstInGroup=false', (tester) async {
      await tester.pumpWidget(wrap(
        Column(
          children: [
            MessageBubble(
    ...
    testWidgets('falls back when location metadata is missing',
        (tester) async {
      final msg = ChatMessage(
        id: 'loc1',
        from: 'u1',
  ...
  group('Read receipt avatars', () {
    testWidgets('renders ReadReceiptAvatars when readReceiptUsers is non-empty',
        (tester) async {
      final msg = makeMessage();
      await tester.pumpWidget(wrap(
        MessageBubble(
          ...
          readReceiptUsers: const [
            ChatUser(id: 'bob', displayName: 'Bob'),
          ],
          readReceipts: [
            ReadReceipt(
              userId: 'bob',
