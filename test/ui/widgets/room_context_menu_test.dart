...
void main() {
  ...
  group('RoomContextMenu', () {
    testWidgets('shows mute for unmuted room', (tester) async {
      const room = RoomListItem(id: 'r1', muted: false, unreadCount: 1);
      await tester.pumpWidget(wrap(const RoomContextMenu(room: room)));
      expect(find.text('Mute'), findsOneWidget);
      expect(find.text('Unmute'), findsNothing);
    });

    testWidgets('shows unmute for muted room', (tester) async {
      const room = RoomListItem(id: 'r1', muted: true);
      await tester.pumpWidget(wrap(const RoomContextMenu(room: room)));
