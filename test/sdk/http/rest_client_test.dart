...
void main() {
  ...
  group('resolveUrl', () {
    test('joins an absolute-path signed URL onto the base origin without '
    ...
    test('leaves an already-absolute URL on a different host unchanged', () {
      final resolved = rest.resolveUrl('https://cdn.example.com/att-1?sig=abc');
  ...
  group('happy paths', () {
    ...
    test(
      'uploadBinary() sets a sendTimeout independent from requestTimeout '
      '(regression: was inheriting the 30s generic requestTimeout, cutting '
      'off large attachment uploads on slow connections)',
      () async {
        when(
          () => dio.request(
            any(),
    ...
    test(
      'downloadBinary() sets a receiveTimeout independent from '
      'requestTimeout',
      () async {
        when(
          () => dio.request(
