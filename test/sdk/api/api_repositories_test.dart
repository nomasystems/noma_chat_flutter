...
void main() {
  ...
  group('MessagesApi', () {
    ...
    test(
      'list() gets /rooms/{roomId}/messages and returns PaginatedResponse',
      () async {
        when(
  ...
  group('MembersApi', () {
    ...
    test(
      'add() posts to /rooms/{roomId}/users with userIds and mode',
      () async {
        when(
          () => rest.postVoid(any(), data: any(named: 'data')),
        ).thenAnswer((_) async {});

        final result = await api.add(
          'r1',
          userIds: ['u1', 'u2'],
  ...
  group('ContactsApi', () {
    ...
    test(
      'list() gets /contacts and returns PaginatedResponse<ChatContact>',
      () async {
        when(
          () => rest.getWithTotalCount(
            '/contacts',
    ...
    test('list() passes pagination params', () async {
      ...
      await api.list(pagination: const PaginationParams(limit: 20, offset: 10));
