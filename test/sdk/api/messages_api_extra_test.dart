...
void main() {
  ...
  group('MessagesApi extra coverage', () {
    test('get() hits GET /rooms/{r}/messages/{m}', () async {
      when(() => rest.get('/rooms/r1/messages/m1'))
    ...
    test('update() puts to /rooms/{r}/messages/{m} with text + metadata',
        () async {
      when(() => rest.putVoid(any(), data: any(named: 'data')))
          .thenAnswer((_) async {});

      final r = await api.update('r1', 'm1',
          text: 'edited', metadata: {'k': 'v'});

      expect(r.isSuccess, true);
      final captured = verify(() => rest.putVoid(
              '/rooms/r1/messages/m1', data: captureAny(named: 'data')))
          .captured
    ...
    test('markRoomAsRead() posts to /rooms/{r}/read', () async {
      ...
      expect(r.isSuccess, true);
      final captured = verify(() => rest.postVoid('/rooms/r1/read',
    ...
    test('getRoomReceipts() hits /rooms/{r}/receipts and maps list',
        () async {
    ...
    test('sendTyping() falls back to PUT /rooms/{r}/users/{u}/activity '
      ...
      expect(r.isSuccess, true);
      verify(() => rest.putVoid('/rooms/r1/users/u1/activity',
          data: any(named: 'data'))).called(1);
    });

    test('sendTyping() returns ValidationFailure if userId is unknown',
        () async {
    ...
    test('getThread() hits /rooms/{r}/messages/{m}/thread', () async {
      ...
      verify(() => rest.getWithTotalCount('/rooms/r1/messages/m1/thread',
    ...
    test('deleteReaction() calls DELETE /rooms/{r}/messages/{m}/reactions',
        () async {
    ...
    test('search() hits /messages/search with q + roomId', () async {
      ...
      expect(r.isSuccess, true);
      final captured = verify(() => rest.getWithTotalCount('/messages/search',
    ...
    test('cancelScheduled() deletes /rooms/{r}/scheduled-messages/{s}',
        () async {
      when(() => rest.delete(any())).thenAnswer((_) async {});
