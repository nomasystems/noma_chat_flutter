...
void main() {
  ...
  test('a queued sendMessage is sent on the very first ConnectedEvent, '
      'without waiting for a reconnect', () async {
  ...
  test('catch-up does NOT fire on the first ConnectedEvent (nothing to catch '
      'up on a session that never disconnected)', () async {
    when(
  ...
  test('catch-up fires after a real disconnect/reconnect cycle', () async {
    when(
      () => rest.get(any(), queryParams: any(named: 'queryParams')),
    ).thenAnswer((_) async => {'rooms': []});
  ...
  test('flushPendingOperations drains the queue on demand, without a '
  ...
  test('pendingOperationCount and flushPendingOperations are no-ops without '
      'an offline queue configured', () async {
