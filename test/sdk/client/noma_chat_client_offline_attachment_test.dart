...
void main() {
  ...
  test('a NetworkFailure during upload queues the attachment and replays it '
      'on reconnect, preserving bytes and reconciling via '
      'onOfflineMessageSent (R2-16)', () async {
    when(
      () => rest.uploadBinary(
        any(),
        any(),
  ...
  test('an attachment at or under offlineQueueMaxAttachmentBytes is queued '
      'normally (C1 boundary)', () async {
    config = ChatConfig(
      baseUrl: 'http://h/v1',
      realtimeUrl: 'http://h',
      tokenProvider: () async => 't',
  ...
  test('without an offline queue configured, enqueueOfflineAttachment is a '
      'no-op instead of throwing', () async {
